...
import {movie, Props} from "./PlaylistButtonType"

const PlaylistButton = ({playlist, movieID, fetchPlaylists}: Props) => {
    const [isAlreadyIn, setIsAlreadyIn] = useState<movie | null>(null);
    const fetchListMoviesIn = async () => {
        try {
            ...
            setIsAlreadyIn(data.find((movieInPlaylist: movie) => movieInPlaylist?.attributes?.movie_id === movieID?.toString()))
        } catch (e: any) {
